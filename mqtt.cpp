#include <mqtt.h>
	...
	void mqtt_publish_message(uint8_t * mqtt_message, char * topic, char * message) {
		...
		void mqtt_subscribe_message(uint8_t * mqtt_message, char * topic) {

        uint8_t i = 0;
        uint8_t topic_length = strlen(topic);

		mqtt_message[0] = 130;                                   // MQTT Message Type CONNECT
		mqtt_message[1] = 5 + topic_length;					    // Remaining length
		mqtt_message[2] = 00;
		mqtt_message[3] = 02;
		mqtt_message[4] = 00;                                   // MQTT Message Type CONNECT
		mqtt_message[5] = topic_length;                         // MQTT Message Type CONNECT
		...
		mqtt_message[6+topic_length]=01;
	}	
